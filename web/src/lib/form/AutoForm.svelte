<script lang="ts">
	...
	async function handleSubmit(event: Event) {
		...
		alert('Not implemented yet: save form data to DB (CWB_AFM_076)');
		/*
        isSubmitting = true;
	...
	<form onsubmit={handleSubmit}>
		<div class="form-grid">
			{#each items as item, i}
				{#if items[i].Comp}
					<item.Comp itemDef={items[i]} bind:this={loadedComponents[i]} bind:formData />
				{:else}
		...
		<div class="form-actions">
			<button type="button" onclick={handleCancel} class="cancel-btn" disabled={isSubmitting}>
				Cancel
			</button>
