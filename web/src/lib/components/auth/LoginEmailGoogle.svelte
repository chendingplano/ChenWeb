<script lang="ts">
	...
	// The default login screen (config value "login-email-google"): email /
	// password, forgot-password, sign-up, optional phone link, Google/GitHub.
	// Reached via the LoginPanel selector, never imported directly. Render for
	// a full-page login (`variant="page"`, the default) or inside a modal
	// (`variant="modal"`), which shows its own "Cancel" button wired to
	// `onClose`. Do not fork this component — add props here instead.
	let {
		variant = 'page',
		onClose
