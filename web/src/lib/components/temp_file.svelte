...
<script lang="ts">
	import type { QueryCondition, Field, Operator, LogicalOperator } from '$lib/types/types';
	...
	let { conditions = $bindable([]), onquery = () => {}, onreset = () => {} } = $props();
	...
	function handleReset() {
		conditions.length = 0;
		logicalOperator = 'AND'; // reset to default
		onreset();
	}
</script>

<!-- 
  The bind is not working. Will solve the problem later (Chen Ding, 2026/01/01)
          <input
            type="radio"
            name="logic"
            value="AND"
            bind:group={logicalOperator === 'AND'}
            on:change={() => logicalOperator = 'AND'}
            class="text-blue-600"
          />
          ...
        <label class="inline-flex items-center">
          <input
            ...
            name="logic"
            value="OR"
            bind:group={logicalOperator === 'OR'}
            on:change={() => logicalOperator = 'OR'}
            class="text-blue-600"
          />
	...
	{#if conditions.length > 0}
		<div class="mt-4 rounded-md border border-gray-200 bg-gray-50 p-3">
			<h3 class="mb-2 text-sm font-medium text-gray-900">
				Active Filters ({conditions.length})
				{#if conditions.length > 1}
					<span class="ml-2 text-xs text-gray-500">(combined with {logicalOperator})</span>
				{/if}
			</h3>
			<ul class="space-y-1">
