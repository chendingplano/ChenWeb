<script lang="ts">
	...
	import {
		listKbInventoryItems,
		getKbInput,
		getRawLines,
		type KbInputRecord,
		type KbInventoryItemRecord,
	...
	let items = $state<KbInventoryItemRecord[]>([]);
	...
	const FOCUS_PDF_WIDTH_KEY = 'inventory-items:focus-pdf-width';
	...
	function normalizeItemSpans(m: KbInventoryItemRecord | undefined): NormalizedSpan[] {
	...
	let allKeywords = $derived.by(() => {
		...
		for (const m of items) {
			if (m.item_category) set.add(m.item_category);
			for (const a of m.aliases ?? []) if (typeof a === 'string' && a.trim()) set.add(a);
		}
	...
	let allCategories = $derived.by(() => {
		const set = new Set<string>();
		for (const m of items) {
			if (m.item_category && m.item_category.trim()) set.add(m.item_category);
		}
		return [...set].sort((a, b) => a.localeCompare(b));
	});
	...
	function fmtSpec(spec: InventoryItemSpec): string {
		const name = (spec?.name ?? '').toString().trim();
		const value = spec?.value == null ? '' : String(spec.value).trim();
		const unit = (spec?.unit ?? '').toString().trim();
		const rhs = [value, unit].filter((x) => x !== '').join(' ');
		if (name && rhs) return `${name}: ${rhs}`;
		return name || rhs;
	}

	function buildItemGroupAttrs(
		m: KbInventoryItemRecord,
		...
		const specStrs = (arr: unknown): string[] =>
			(Array.isArray(arr) ? (arr as InventoryItemSpec[]) : [])
				.map((s) => fmtSpec(s))
				.filter((s) => s !== '');
	...
	let itemMap = $derived.by((): ItemCanvas | null => {
		...
		const m = items.find((x) => x.id === selectedItemId);
		if (!m) return null;
		const spans = normalizeItemSpans(m);
		const attrsByGroup = buildItemGroupAttrs(m, spans, rawLineByKey);
	...
	let filteredItems = $derived.by(() => {
		...
		if (kw) {
			result = result.filter(
				(m) =>
					...
					(m.canonical_name ?? '').toLowerCase().includes(kw) ||
					(m.item_category ?? '').toLowerCase().includes(kw) ||
		...
		if (cat) {
			result = result.filter((m) => (m.item_category ?? '').toLowerCase().includes(cat));
		}
	...
	function onItemCardClick(event: MouseEvent, m: KbInventoryItemRecord) {
		event.preventDefault();
	...
	function itemNameOf(m: KbInventoryItemRecord): string {
		return (
	...
	function spanCount(m: KbInventoryItemRecord): number {
		const raw = (m as { source_line_spans?: unknown })?.source_line_spans;
						...
						{#each filteredItems as m, idx (m.id)}
							...
							>
								...
								<div class="card-body">
									...
									{#if m.evidence_quote}
										<div class="card-desc">{m.evidence_quote}</div>
									{/if}
									<div class="card-foot">
										<span class="chip">
