<script lang="ts">
	...
	import SemanticRetryQueueView from '$lib/components/home3/semantic-retry-queue-view.svelte';
	...
	import BenchmarkSetupView from '$lib/components/home3/benchmark-setup-view.svelte';
	...
	let showFooter = $derived(
		sectionId !== 'chat' &&
			...
			activeMenu?.childId !== 'sysadmin-doc-process-semantic-assertions' &&
...
>
	...
	<div class="flex-1 {showFooter ? '' : 'min-h-0'}">
		...
		{:else if activeMenu?.childId === 'sysadmin-doc-process-semantic-retry-queue'}
			<SemanticRetryQueueView {darkMode} />
		...
		{:else if activeMenu?.childId === 'sysadmin-benchmark-setup'}
			<BenchmarkSetupView {darkMode} />
