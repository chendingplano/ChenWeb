<script lang="ts">
	...
	async function openJsonModal() {
		...
		try {
			const res = await fetch(`/api/v1/doc-review/reports/${linkReportId}`, {
	...
	async function openMdModal() {
		...
		mdModalLoading = true;
		try {
			const res = await fetch(`/api/v1/doc-review/reports/${linkReportId}/export?format=md`, {
				credentials: 'same-origin'
			});
	...
	async function fetchPDFFiles() {
		...
		pdfLoading = true;
		try {
			const res = await fetch(`/api/v1/doc-review/reports/${linkReportId}/pdf-files`, {
				credentials: 'same-origin'
			});
			const data = await res.json();
			pdfFiles = data.files || [];
	...
	let severitySlices = $derived(
		buildDonutSlices(
			severityItems.filter((i) => !deselectedSeverities.has(i.label)),
			35
		)
	);
	...
	let packageChartData = $derived.by(() => {
		...
		return {
			slices: buildDonutSlices(
				activeItems.map((p) => ({ count: p.count, color: p.color, label: p.label })),
				35
			),
			nonEmpty,
			emptyLabels
	...
	let reviewerChartData = $derived.by(() => {
		...
		return {
			slices: buildDonutSlices(
				activeItems.map((r) => ({ count: r.count, color: r.color, label: r.label })),
				35
			),
			nonEmpty,
			emptyLabels
	...
	let elapsedSeconds = $derived.by(() => {
		...
		return Math.round(
			(new Date(request.end_time).getTime() - new Date(request.start_time).getTime()) / 1000
		);
	});
		...
		try {
			...
			if (viewStatus === 'completed' || viewStatus === 'failed' || viewStatus === 'stopped') {
				...
				if (viewStatus === 'completed' && rid) {
					try {
						const res = await fetch(`/api/v1/doc-review/reports/${rid}`, {
							credentials: 'same-origin'
						});
						const data = await res.json();
...
{:else}
	<div style="padding: 1.5rem;">
		...
		{#if error}
			<div
				style="margin-bottom: 1rem; padding: 0.75rem 1rem; background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.25); border-radius: 10px; color: #fca5a5; font-size: 0.9rem;"
			>
				{error}
			</div>
		{/if}
		...
		{:else if viewStatus === 'failed'}
			<!-- Failed State -->
			<div
				style="background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.3); border-radius: 12px; padding: 2rem; text-align: center;"
			>
		...
		{:else if viewStatus === 'stopped'}
			<!-- Stopped State -->
			<div
				style="background: rgba(245,158,11,0.1); border: 1px solid rgba(245,158,11,0.3); border-radius: 12px; padding: 2rem; text-align: center;"
			>
		...
		{:else}
			...
			{#if activeTab === 'findings'}
				<div style="height: calc(100vh - 220px); min-height: 640px;">
					<DocReviewFindingsView
						{darkMode}
						initialRunId={currentRunId > 0 ? String(currentRunId) : ''}
						embedded={true}
					/>
				</div>
			...
			{:else}
				...
				>
					...
					<div
						style="background: {cardBg}; border: 1px solid {borderColor}; border-radius: 12px; padding: 1rem;"
					>
						<div
							...
							<svg viewBox="0 0 100 100" width="304" height="304" style="flex-shrink: 0;">
								...
								{#each severitySlices as s}
									...
									<circle
										...
										onmouseenter={() =>
											(hoverSev = { label: s.label, count: s.count, color: s.color })}
								...
								{#if hoverSev}
									<text
										...
										font-weight="800"
										fill={hoverSev.color}
										style="pointer-events: none;">{hoverSev.count}</text
									>
									<text
										x="50"
										y="57"
										...
										font-size="4.5"
										fill={textSecondary}
										style="pointer-events: none;">{hoverSev.label}</text
									>
								{/if}
							</svg>
							<div
								style="display: flex; flex-direction: column; gap: 0.55rem; flex: 1; min-width: 0;"
							>
								{#each severityItems as item}
									{@const isOff = deselectedSeverities.has(item.label)}
									<!-- svelte-ignore a11y_no_static_element_interactions -->
									<div
										onclick={() => toggleSeverity(item.label)}
										onkeydown={(e) => {
											if (e.key === 'Enter' || e.key === ' ') toggleSeverity(item.label);
										}}
										role="button"
										tabindex="0"
										style="display: flex; align-items: center; gap: 0.5rem; font-size: 0.85rem; cursor: pointer; user-select: none; opacity: {isOff
											? 0.35
											: 1}; transition: opacity 0.15s;"
									>
										<span
											style="width: 12px; height: 12px; border-radius: 2px; background: {isOff
												? textMuted
												: item.color}; flex-shrink: 0; transition: background 0.15s;"
										></span>
										<span
											style="color: {textSecondary}; flex: 1; text-decoration: {isOff
												? 'line-through'
												: 'none'};">{item.label}</span
										>
										<span style="font-weight: 700; color: {isOff ? textMuted : item.color};"
											>{item.count}</span
										>
									</div>
								{/each}
						...
						<div style="display: flex; align-items: center; gap: 1.25rem;">
							<svg viewBox="0 0 100 100" width="304" height="304" style="flex-shrink: 0;">
								...
								{#each packageChartData.slices as s}
									...
									<circle
										...
										style="cursor: pointer;"
										onmouseenter={() =>
											(hoverPkg = { label: s.label, count: s.count, color: s.color })}
								...
								{#if hoverPkg}
									...
									<text
										x="50"
										y={pkgParts.length > 1 ? '40' : '44'}
										text-anchor="middle"
										dominant-baseline="central"
										font-size="9"
										font-weight="800"
										fill={hoverPkg.color}
										style="pointer-events: none;">{hoverPkg.count}</text
									>
									<text
										x="50"
										y={pkgParts.length > 1 ? '51' : '57'}
										text-anchor="middle"
										dominant-baseline="central"
										font-size="4.5"
										fill={textSecondary}
										style="pointer-events: none;">{pkgParts[0]}</text
									>
									{#if pkgParts.length > 1}
										<text
											...
											font-size="4.5"
											fill={textSecondary}
											style="pointer-events: none;">{pkgParts[1]}</text
										>
									{/if}
								{/if}
							</svg>
							<div style="flex: 1; min-width: 0; overflow-y: auto; max-height: 280px;">
								{#each packageChartData.nonEmpty as item}
									...
									<!-- svelte-ignore a11y_no_static_element_interactions -->
									<div
										onclick={() => togglePackage(item.key)}
										onkeydown={(e) => {
											if (e.key === 'Enter' || e.key === ' ') togglePackage(item.key);
										}}
										role="button"
										tabindex="0"
										style="display: flex; align-items: center; gap: 0.45rem; font-size: 0.8rem; margin-bottom: 0.35rem; cursor: pointer; user-select: none; opacity: {isOff
											? 0.35
											: 1}; transition: opacity 0.15s;"
									>
										<span
											style="width: 9px; height: 9px; border-radius: 2px; background: {isOff
												? textMuted
												: item.color}; flex-shrink: 0; transition: background 0.15s;"
										></span>
										<span
											style="color: {textSecondary}; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; text-decoration: {isOff
												? 'line-through'
												: 'none'};">{item.label}</span
										>
										<span style="font-weight: 600; color: {isOff ? textMuted : textPrimary};"
											>{item.count}</span
										>
									</div>
								{/each}
						...
						<div style="display: flex; align-items: center; gap: 1.25rem;">
							<svg viewBox="0 0 100 100" width="304" height="304" style="flex-shrink: 0;">
								...
								{#each reviewerChartData.slices as s}
									...
									<circle
										...
										style="cursor: pointer;"
										onmouseenter={() =>
											(hoverRev = { label: s.label, count: s.count, color: s.color })}
								...
								{#if hoverRev}
									...
									<text
										x="50"
										y={revParts.length > 1 ? '40' : '44'}
										text-anchor="middle"
										dominant-baseline="central"
										font-size="9"
										font-weight="800"
										fill={hoverRev.color}
										style="pointer-events: none;">{hoverRev.count}</text
									>
									<text
										x="50"
										y={revParts.length > 1 ? '51' : '57'}
										text-anchor="middle"
										dominant-baseline="central"
										font-size="4.5"
										fill={textSecondary}
										style="pointer-events: none;">{revParts[0]}</text
									>
									{#if revParts.length > 1}
										<text
											...
											font-size="4.5"
											fill={textSecondary}
											style="pointer-events: none;">{revParts[1]}</text
										>
									{/if}
								{/if}
							</svg>
							<div style="flex: 1; min-width: 0; overflow-y: auto; max-height: 280px;">
								{#each reviewerChartData.nonEmpty as item}
									...
									<!-- svelte-ignore a11y_no_static_element_interactions -->
									<div
										onclick={() => toggleReviewer(item.aspect)}
										onkeydown={(e) => {
											if (e.key === 'Enter' || e.key === ' ') toggleReviewer(item.aspect);
										}}
										role="button"
										tabindex="0"
										style="display: flex; align-items: center; gap: 0.45rem; font-size: 0.8rem; margin-bottom: 0.35rem; cursor: pointer; user-select: none; opacity: {isOff
				...
				>
					...
					<div style="position: relative;" onmouseleave={closePDFMenu}>
						<button
							...
							style="padding: 0.4rem 0.75rem; background: {pdfMenuOpen
								? accent
								: accentTint}; color: {pdfMenuOpen
								? '#fff'
							...
							>
								...
								{:else}
									...
									{#each pdfFiles as f (f.report_id)}
										<a
											href={`/api/v1/doc-review/reports/${f.report_id}/export?format=pdf`}
											target="_blank"
											onclick={closePDFMenu}
											style="display: flex; align-items: center; gap: 0.6rem; padding: 0.6rem 1rem; color: {f.is_current
												? accent
												: textPrimary}; text-decoration: none; font-size: 0.85rem; background: {f.is_current
												? accentTint
												: 'transparent'}; border-bottom: 1px solid {borderColor};"
										>
											<span
												style="flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;"
												title={f.file_name}>{f.file_name}</span
											>
											{#if f.is_current}
												<span
					...
					<a
						href={`/api/v1/doc-review/reports/${linkReportId}/export?format=pdf`}
						download
					...
					<a
						href={`/api/v1/doc-review/reports/${linkReportId}/export?format=md`}
						download
						style="padding: 0.4rem 0.75rem; background: {accentTint}; color: {accent}; border-radius: 6px; text-decoration: none; font-size: 0.85rem;"
					>
					...
					<button
						...
						style="padding: 0.4rem 1rem; background: transparent; color: {isRerunning
							? textMuted
...
{#if rerunDialogOpen && request}
	...
	<div
		onclick={(e) => {
			if (e.target === e.currentTarget) closeRerunDialog();
		}}
		onkeydown={(e) => {
			if (e.key === 'Escape') closeRerunDialog();
		}}
		role="dialog"
		aria-modal="true"
		tabindex="-1"
	...
	>
		<div
			style="background: {cardBg}; border: 1px solid {borderColor}; border-radius: 14px; width: min(100%, 720px); box-shadow: 0 24px 64px rgba(0,0,0,0.45); overflow: hidden;"
		>
			...
			<div style="padding: 0.5rem 1.5rem 1.25rem;">
				<div
					style="display: grid; grid-template-columns: minmax(140px, 180px) 1fr; gap: 0.65rem 1.25rem; padding: 1rem 1.1rem; background: {darkMode
						? 'rgba(15,23,42,0.35)'
			...
			<div style="display: flex; gap: 0.75rem; padding: 0 1.5rem 1.5rem;">
				...
				<button
					...
					disabled={isRerunning}
					style="flex: 1; padding: 0.85rem 1.25rem; background: {accent}; color: #fff; border: none; border-radius: 10px; cursor: {isRerunning
						? 'default'
						: 'pointer'}; font-size: 1rem; font-weight: 700; display: flex; align-items: center; justify-content: center; gap: 0.5rem;"
				>
					{#if isRerunning}
	...
	>
		<div
			style="background: {darkMode
				? '#161B27'
				: '#FFFFFF'}; border: 1px solid {borderColor}; border-radius: 14px; width: min(90vw, 880px); max-height: 85vh; display: flex; flex-direction: column; box-shadow: 0 24px 64px rgba(0,0,0,0.5);"
		>
			<!-- Modal header -->
			<div
				style="display: flex; align-items: center; justify-content: space-between; padding: 1rem 1.25rem; border-bottom: 1px solid {borderColor}; flex-shrink: 0;"
			>
				<div>
				...
				<div style="display: flex; align-items: center; gap: 0.5rem;">
					<a
						href={`/api/v1/doc-review/reports/${linkReportId}`}
						target="_blank"
					...
					<button
						...
						style="padding: 0.3rem 0.65rem; background: transparent; color: {textMuted}; border: 1px solid {borderColor}; border-radius: 6px; cursor: pointer; font-size: 0.8rem; line-height: 1;"
						>✕</button
					>
				</div>
			</div>
			<!-- Modal body -->
			<div
				style="overflow: auto; padding: 1.25rem; font-family: 'JetBrains Mono', 'Fira Code', 'Cascadia Code', 'Consolas', monospace; font-size: 0.82rem; line-height: 1.6; flex: 1;"
			>
				{#if jsonModalLoading}
					<div
...
{#if mdModalOpen}
	...
	>
		<div
			style="background: {darkMode
				? '#161B27'
				: '#FFFFFF'}; border: 1px solid {borderColor}; border-radius: 14px; width: min(90vw, 900px); max-height: 85vh; display: flex; flex-direction: column; box-shadow: 0 24px 64px rgba(0,0,0,0.5);"
		>
			<!-- Modal header -->
			<div
				style="display: flex; align-items: center; justify-content: space-between; padding: 1rem 1.25rem; border-bottom: 1px solid {borderColor}; flex-shrink: 0;"
			...
			<div
				class="md-body"
				style="overflow: auto; padding: 1.5rem 2rem; flex: 1; color: {textPrimary};"
			>
				{#if mdModalLoading}
					<div
						style="display: flex; align-items: center; gap: 0.5rem; color: {textSecondary}; padding: 2rem; justify-content: center;"
...
{#if pendingConfirm}
	...
	<div
		onclick={(e) => {
			if (e.target === e.currentTarget && !translating[confirmId]) cancelTranslate();
		}}
		onkeydown={(e) => {
			if (e.key === 'Escape' && !translating[confirmId]) cancelTranslate();
		}}
		role="dialog"
		aria-modal="true"
		tabindex="-1"
	...
	>
		<div
			style="background: {darkMode
				? '#161B27'
				: '#FFFFFF'}; border: 1px solid {borderColor}; border-radius: 14px; width: min(90vw, 420px); box-shadow: 0 24px 64px rgba(0,0,0,0.5); padding: 1.5rem;"
		>
			<div style="font-weight: 600; font-size: 1rem; color: {textPrimary}; margin-bottom: 0.5rem;">
				{copy.title}
			</div>
			{#if pendingFinding}
				<div
					style="font-size: 0.85rem; color: {textSecondary}; margin-bottom: 1rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"
				>
					{pendingFinding.title}
				</div>
			{/if}
			{#if translating[pendingConfirm.id]}
				<div
					style="display: flex; align-items: center; gap: 0.6rem; color: {textSecondary}; font-size: 0.9rem; padding: 0.5rem 0;"
				>
					<LoaderIcon size={18} style="animation: spin 1s linear infinite; color: {accent};" />
			...
			{:else}
				...
				<div style="display: flex; justify-content: flex-end; gap: 0.5rem;">
					<button
						...
						style="padding: 0.4rem 0.9rem; background: transparent; color: {textMuted}; border: 1px solid {borderColor}; border-radius: 8px; cursor: pointer; font-size: 0.85rem;"
						>{copy.cancel}</button
					>
					<button
						onclick={() => pendingFinding && confirmTranslate(pendingFinding)}
						style="padding: 0.4rem 0.9rem; background: {successBg}; color: #22c55e; border: none; border-radius: 8px; cursor: pointer; font-size: 0.85rem;"
						>{copy.translate}</button
					>
				</div>
			{/if}
		</div>
