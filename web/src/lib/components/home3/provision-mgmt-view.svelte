<script lang="ts">
	...
	let metrics = $state<KbProvisionRecord[]>([]);
	...
	let extractedMetricsPreview = $state<ExtractedKbProvision[]>([]);
	...
	function clampFocusPdfWidth(value: number) {
		...
		// Cap the PDF panel at 75% of the viewport so the Info Panel always has
		// at least 25% of the window width.
	...
	function normalizeMetricSpans(m: KbProvisionRecord | undefined): NormalizedSpan[] {
		...
		for (const item of raw) {
			if (typeof item === 'string') {
				...
				pushLine(Math.trunc(item));
			} else if (item && typeof item === 'object') {
				const obj = item as Record<string, unknown>;
				const l = toPositiveInt(obj.line_number ?? obj.line ?? obj.line_no ?? obj.lineNo);
				const p = toPositiveInt(obj.page_number ?? obj.page ?? obj.page_no ?? obj.pageNo);
				if (l) pushLine(l, p);
			}
		}
		return out;
	...
	// ---------- Keyword filter + provision name nav ----------
	let allKeywords = $derived.by(() => {
		const set = new Set<string>();
		for (const m of metrics) {
			for (const kw of m.provision_keywords ?? []) set.add(kw);
			for (const kw of m.provision_keywords_en ?? []) set.add(kw);
		}
		return [...set].sort((a, b) => a.localeCompare(b));
	});

	// Flattens kb.provisions.category_paths (which may be an array of path
	// strings, an array of {category_path:[{name}]} payloads, or an array of
	// [{name}] chains) into a list of "a/b/c" path strings for display.
	function provisionCategoryStrings(raw: unknown): string[] {
		if (!Array.isArray(raw)) return [];
		const out: string[] = [];
		const namesOf = (segs: unknown): string => {
		...
		for (const item of raw) {
			if (typeof item === 'string') {
				...
				const path = namesOf(item);
				if (path) out.push(path);
			} else if (item && typeof item === 'object') {
				const obj = item as Record<string, unknown>;
				const path = namesOf(obj.category_path) || String(obj.path ?? obj.name ?? '').trim();
				if (path) out.push(path);
			}
		}
		return out;
	...
	function buildMetricGroupAttrs(
		...
		const kwItems = (m.provision_keywords ?? []).filter(
			(v) => typeof v === 'string' && v.trim() !== ''
		);
		const categoryItems = provisionCategoryStrings(m.category_paths);
	...
	let filteredMetrics = $derived.by(() => {
		...
		if (kw) {
			result = result.filter(
				(m) =>
					(m.provision_keywords ?? []).some((k) => k.toLowerCase().includes(kw)) ||
					(m.provision_keywords_en ?? []).some((k) => k.toLowerCase().includes(kw)) ||
					(m.prov_name ?? '').toLowerCase().includes(kw) ||
					(m.provision_subject ?? '').toLowerCase().includes(kw)
			);
		}
	...
	function onMetricCardClick(event: MouseEvent, m: KbProvisionRecord) {
	...
	function spanCount(m: KbProvisionRecord): number {
		const raw = (m as { source_line_spans?: unknown })?.source_line_spans;
		if (!Array.isArray(raw)) return 0;
		// Each provision span entry ("page:line" string or {page,line} object)
		// references a single source line.
		return raw.length;
	}
	...
	async function extractMetric() {
		...
		try {
			const seen = new Set<string>();
			const spans = addMetricDialogLines
				.map((l) => ({ page_number: l.page_number, line_number: l.line_number }))
				.filter((s) => {
					const key = `${s.page_number}:${s.line_number}`;
					if (seen.has(key)) return false;
					seen.add(key);
					return true;
				});
			const result = await extractKbProvisions({
	...
	async function saveExtractedMetricsPreview() {
		...
		try {
			await saveExtractedKbProvisions({
				record_id: currentInput.id,
				provisions: extractedMetricsPreview
			});
			const refreshed = await listKbProvisions(currentInput.id);
							...
							>
								...
								<div class="card-body">
									...
									{#if m.prov_desc || m.provision}
										<div class="card-desc">{m.prov_desc || m.provision}</div>
									{/if}
									<div class="card-foot">
										<span class="chip">
								...
								{#each extractedMetricsPreview as metric, idx (`${previewMetricNameOf(metric, idx)}-${idx}`)}
									<div class="am-preview-card">
										<div class="am-preview-head">
											<div>
												...
												<div class="am-preview-meta">
													...
													{#if metric.provision_type}<span>{metric.provision_type}</span>{/if}
										...
										{#if metric.prov_desc || metric.provision}
											<div class="am-preview-desc">{metric.prov_desc || metric.provision}</div>
										{/if}
										<div class="am-preview-fields">
											{#if metric.provision_subject}<span class="chip chip-mono"
													>{metric.provision_subject}</span
												>{/if}
											{#if metric.is_explicit != null}<span class="chip chip-quiet"
													>{metric.is_explicit ? 'explicit' : 'implicit'}</span
												>{/if}
										</div>
									</div>
								{/each}
