...
<Sidebar.Provider>
	...
	<main class="flex flex-1 overflow-hidden">
		...
		<Resizable.PaneGroup direction="horizontal" class="h-full w-full">
			<Resizable.Pane defaultSize={75}>
				...
				<div class="flex h-full items-center justify-center p-6">
					{#if activeView.title === 'Documents'}
						<div class="h-[900px] w-full overflow-hidden px-4 pt-5">
							<TableWithFilters
								class="h-full w-full"
					...
					{:else if activeView.title === 'table-03'}
						<div class="h-[800px] overflow-hidden px-4 pt-5">
							<Table03 class="h-full w-full" />
						</div>
					{:else if activeView.title === 'Login'}
						<div class="h-[500px] w-[200px] items-center justify-center px-4 pt-5">
							<LoginPage />
						</div>
					{:else if activeView.title === 'Dashboard' || !activeView}
						<div class="h-[900px] w-full overflow-auto px-4 pt-5">
							<Dashboard01 />
						</div>
					{:else}
						<div class="flex h-full items-center justify-center">
							<span class="font-semibold text-muted-foreground">
								'{activeView.title}' menu not implemented yet!
							</span>
						</div>
					{/if}
				</div>
			...
			<Resizable.Pane defaultSize={25} minSize={15} class="overflow-auto bg-muted">
				<div class="h-full border-b p-4">
					...
					{:else if activeView}
						...
						<p class="mt-2 text-sm text-muted-foreground">No details available for this view.</p>
					{:else}
						<h2 class="text-lg font-semibold">Details</h2>
					{/if}
				</div>
