<script lang="ts">
	...
	const _tokens = { shadowCard, shadowCardDark, shadowHover, shadowDropdown, radiusButton, radiusChip, durationHover, durationAurora };
	void _tokens;
	...
	const _railCollapsed = RAIL_WIDTH_COLLAPSED; void _railCollapsed;
	...
	<div class="flex flex-1 overflow-hidden select-none">
		...
		{#if railPinned}
			<div
				...
				onmouseenter={(e) => { (e.currentTarget as HTMLElement).style.background = accent + '50'; }}
		...
		<ContentPanel {darkMode} {activeMenu} {shelfOpen} onToggleShelf={() => { shelfOpen = !shelfOpen; }}
