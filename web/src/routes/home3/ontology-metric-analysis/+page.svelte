	...
	<main class="content">
		...
		<section class="dashboard-grid">
			...
			<div class="panel errors-panel">
				...
				<div class="error-bars">{#each errors as error}<div class="error-bar-row"><div class="error-bar-meta"><span>{error.label}</span><span><b>{error.value}</b> affected <em>{error.severity}</em></span></div><div class="error-track"><span style={`width:${error.percent}%`}></span></div></div>{:else}<div class="empty-state">(no data)</div>{/each}</div>
...
<style>
	...
	.empty-state { padding: 24px 0; color: var(--subtle); font: 500 10px 'DM Mono', monospace; text-align: center; }
