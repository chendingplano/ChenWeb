	...
	<Sidebar.Inset>
		...
		<div class="flex flex-1 flex-col gap-4 p-4 pt-0">
			<div class="grid auto-rows-min gap-4 md:grid-cols-3">
				<div class="aspect-video rounded-xl bg-muted/50">Round box 1</div>
				<div class="aspect-video rounded-xl bg-muted/50">Round box 2</div>
				<div class="aspect-video rounded-xl bg-muted/50">Round box 3</div>
			</div>
			<div class="min-h-[100vh] flex-1 rounded-xl bg-muted/50 md:min-h-min">
				...
				{:else if activeView.title === 'History'}
					<div class="h-[800px] overflow-hidden px-4 pt-5">History not implemented yet!</div>
				{:else}
					<div class="flex h-full items-center justify-center">
						<span class="font-semibold text-muted-foreground"
							>'{activeView.title}' menu not implemented yet 02!</span
						>
					</div>
				{/if}
			</div>
