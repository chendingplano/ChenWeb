...
SHALL serve a cached rendering when one exists for that version.
...
### Requirement: Live preview renders an unsaved draft without persistence

`POST /api/v1/cdm/documents/:key/render-preview` SHALL accept canonical
document JSON and return paginated SVG pages without saving the document,
advancing `edit_version`, or writing persistent rendering artifacts. Request
cancellation SHALL cancel its Typst compilation.

#### Scenario: Unsaved content is previewed

- **WHEN** the editor sends locally modified canonical document JSON
- **THEN** the returned pages reflect those modifications and the stored
  document remains unchanged

#### Scenario: A preview is superseded

- **WHEN** the client cancels an in-flight preview because the author edits
  again
