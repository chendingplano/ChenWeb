...
`image`, and `quote`, and the inline types `text`, `line_break`, `strong`,
`emphasis`, `code`, `link`, `math`, `citation`, `cross_reference`. Output SHALL
begin by importing the theme file and emitting the document title as a
top-level heading.
...
#### Scenario: Inline line breaks render as Typst line breaks

- **WHEN** a `paragraph` inline sequence contains `text`, `line_break`, `text`
- **THEN** the renderer emits `#linebreak()` between the two text runs
