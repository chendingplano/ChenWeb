...
types `text`, `strong`, `emphasis`, `code`, `link`, `math`, `citation`,
`cross_reference`. A `callout` block MAY carry a `role` drawn from `note`, `tip`,
`important`, `warning`, `caution`. `warning` SHALL NOT be accepted as a block
type.
