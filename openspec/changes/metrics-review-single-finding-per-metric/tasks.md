...
- [ ] 1.1 Copy `ChenWeb/prompts/prompt-review-metrics-v5.md` to
      `ChenWeb/prompts/prompt-review-metrics-v6.md` (leave v5 untouched, per prompt-file
      convention).
- [ ] 1.2 In v6, add a top-level `metric_summary` output field (2-4 sentences, one-time
      explanation of the metric under review) to §6 Output Format and §1/§3 wherever the
      contract is described.
- [ ] 1.3 In v6 §3/§6, change `analyses[].summary` guidance so it assumes the reader already has
...
- [ ] 1.4 Update the JSON example in §6 to include `metric_summary` alongside `analyses` and
      `findings`.

## 2. Storage model

- [ ] 2.1 In `server/api/doc-reviews/review-document.go`, add
...
- [ ] 2.2 In `server/api/doc-reviews/models.go`, add `RelatedArtifacts []RelatedArtifactAnalysis`
...
- [ ] 2.3 In `server/api/doc-reviews/finding_translation.go`, thread `finding.RelatedArtifacts`
...
- [ ] 2.4 In `server/api/doc-reviews/models.go`, decide/implement whether `applyFindingMetadata`
...
- [ ] 3.1 In `server/api/doc-reviews/review-metrics.go`, add `parseMetricSummaryJSON(payload
      map[string]any) string` alongside `parseMetricAnalysesJSON`.
- [ ] 3.2 Replace `metricAnalysesAsFindings(dm docMetric, analyses []MetricAnalysis)
...
- [ ] 3.3 Update `reviewMetric` (around line 283) to call the new function and append at most one
      finding, instead of appending N.
- [ ] 3.4 Review the per-finding post-processing loop (`review-metrics.go:295-314`, the
...
- [ ] 3.5 Update package doc-comment on `MetricAnalysis`/`metricAnalysesAsFindings` (now
      `metricAnalysesAsFinding`) to describe the new one-row shape.

## 4. Config

- [ ] 4.1 In `ChenWeb/doc-review.local.toml`, bump `reviewers.metrics.prompt` to
      `prompt-review-metrics-v6.md`.

## 5. Tests

- [ ] 5.1 Update `TestParseMetricAnalysesJSON` (unchanged expectations — parsing per-entry
      `analyses` is untouched) and add `TestParseMetricSummaryJSON` covering present/absent
      `metric_summary`.
- [ ] 5.2 Update `TestReviewMetric_ReturnsAnalysesAsFindings` to assert exactly one finding is
...
- [ ] 5.3 Add a test: `analyses` present but empty (or `metric_summary` present with zero
      analyses) → zero findings from `metricAnalysesAsFinding`.
- [ ] 5.4 Add/extend a `finding_translation_test.go` or `models_test.go` case round-tripping a
...
- [ ] 5.5 Run `cd ChenWeb/server && go test ./api/doc-reviews/...` and confirm the full package
      passes.

## 6. Verification

- [ ] 6.1 Re-run (or simulate with the existing test fixtures) the `244_mtc_2` case from the bug
