# holiday-calendar-admin Specification

## Purpose
TBD - created by archiving change holiday-calendar-admin. Update Purpose after archive.
...
definitions ("holiday info"), each with a country, name, unique positive per-country display
sequence number, optional description, and optional note. A holiday info SHALL be uniquely
identified by (country, name), and display sequence numbers SHALL be unique within each country.

#### Scenario: New holiday is appended
- **WHEN** an admin creates a holiday definition for a country
- **THEN** the system assigns it one more than the largest display sequence number for that country

#### Scenario: Holiday display order is editable
- **WHEN** an admin changes a holiday definition’s display sequence number
- **THEN** the system moves that holiday to the requested position and shifts affected definitions
  so sequence numbers remain unique and the list displays in sequence order
