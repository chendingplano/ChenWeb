-- extract_products' summary log (LogSummary) has always passed
-- entry_type='extract_products', but doc_proc_logs_entry_type_check was
-- never widened to allow it -- masked until now because extract_products
-- was orphaned (never wired into runtime.go) and so never actually ran.
-- +goose Up
