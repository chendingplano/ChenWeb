-- +goose Up
-- The doc-processing pipeline writes 'reconcile_object' rows (see
-- doc_proc_log_store.go EntryTypeReconcileObject / LogReconcileObject), but the
-- doc_proc_logs_entry_type_check constraint was never widened to allow it, so
-- every reconcile_object insert failed with a 23514 check-constraint violation.
-- Recreate the constraint with 'reconcile_object' added.
