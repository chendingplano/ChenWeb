...
func TestNewTestMethodsProcessor_ConfiguresExtractorFromModelsFile(t *testing.T) {
	tmp := t.TempDir()
...
`
	...
	t.Setenv("EXTRACT_TEST_METHODS_MODEL_NAME", "deepseek-flash-chen")
	t.Setenv("MODEL_DEF_FILE", modelsPath)

	client := &llmclients.OpenAIJSONClient{}
	p := NewTestMethodsProcessor(client)
	...
	if client.APIKey != "sk-test-test-methods" {
		t.Fatalf("extractor APIKey = %q, want sk-test-test-methods (never wired from .models.toml)", client.APIKey)
	}
