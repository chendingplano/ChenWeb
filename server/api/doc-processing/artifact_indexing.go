...
// FindSimilarArtifactsOnTheFly runs a hybrid (lexical + pgvector) search over
// kb.search_artifacts and applies the RRF acceptance policy, computing matches live and
// returning them instead of persisting edges. It is the read-time replacement for the
// (now removed) materialized hybrid_search / semantically_related edges — used by the
// document reviewers' Branch A for metrics, inventory items, provisions, and entities.
