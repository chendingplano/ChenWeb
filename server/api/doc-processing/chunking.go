...
const (
	...
	ChunkingMethodFixed    = "fix-size"
	numericSplitMultiplier = 3
)

var (
	linePattern            = regexp.MustCompile(`^(\d+)\s+(\d+)\s+(\S+)\s+(.*?)\s+(\[[^\]]*\])$`)
...
func NewService(store Store, logger ApiTypes.JimoLogger) *Service {
	if logger == nil {
...
func (s *Service) HandleInput(ctx context.Context, recordID int64, inputFilename string, inputFile []byte) error {
	if s.Store == nil {
...
func parseLine(raw string) (Line, error) {
	m := linePattern.FindStringSubmatch(strings.TrimSpace(raw))
	if len(m) != 6 {
	...
	return Line{
		...
		LineNo:     lineNo,
		PageNo:     pageNo,
		LineType:   strings.ToLower(strings.TrimSpace(m[3])),
		Content:    strings.TrimSpace(m[4]),
		Coordinate: strings.TrimSpace(m[5]),
	}, nil
}
