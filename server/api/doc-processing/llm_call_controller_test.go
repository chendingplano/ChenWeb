...
type fakePermitTimer struct {
	stopped bool
	fire    func()
}

func (t *fakePermitTimer) Stop() bool {
	wasActive := !t.stopped
	t.stopped = true
	return wasActive
}

func (t *fakePermitTimer) Fire() {
...
type fakePermitScheduler struct {
	timers []*fakePermitTimer
}

func (s *fakePermitScheduler) Schedule(_ time.Duration, fn func()) permitTimer {
	timer := &fakePermitTimer{fire: fn}
	s.timers = append(s.timers, timer)
	return timer
}

func (s *fakePermitScheduler) FireNext() {
	if len(s.timers) == 0 {
		return
	}
	timer := s.timers[0]
...
func TestLLMCallControllerBlocksPerModel(t *testing.T) {
	controller := newLLMCallController(llmCallControllerConfig{
		DefaultLimit: 1,
...
func TestLLMCallControllerLeaseExpiryReleasesPermit(t *testing.T) {
	scheduler := &fakePermitScheduler{}
	controller := newLLMCallController(llmCallControllerConfig{
		DefaultLimit: 1,
		LeaseTTL:     320 * time.Second,
