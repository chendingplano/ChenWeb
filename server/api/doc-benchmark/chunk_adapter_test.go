...
func TestChunkAdapterCapturesProductionRunRowAndReconcilesEachChunk(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(ChunkRowsQuery)).WithArgs(int64(1203)).WillReturnRows(
...
func TestChunkAdapterRejectsRepresentationDisagreementAndMissingArtifact(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			mock.ExpectQuery(regexp.QuoteMeta(ChunkRowsQuery)).WithArgs(int64(1)).WillReturnRows(sqlmock.NewRows([]string{"id", "chunking_method", "chunking_size", "overlap_percent", "notes", "overlap_lines", "normal_lines", "chunk_lines", "create_time", "update_time"}).AddRow(1, "fixed", 1, 0, "", `["[]"]`, `["[1-3]"]`, `["x"]`, nil, nil))
			a := ChunkAdapter{DB: db, ArtifactPath: func(int64) string { return path }}
...
func TestChunkAdapterRejectsMalformedProductionRanges(t *testing.T) {
	for _, raw := range []string{`["[4-2]"]`, `["[1, 1]"]`, `["[0]"]`, `["[1-x]"]`} {
		t.Run(raw, func(t *testing.T) {
			...
			mock.ExpectQuery(regexp.QuoteMeta(ChunkRowsQuery)).WillReturnRows(sqlmock.NewRows([]string{"id", "chunking_method", "chunking_size", "overlap_percent", "notes", "overlap_lines", "normal_lines", "chunk_lines", "create_time", "update_time"}).AddRow(1, "fixed", 1, 0, "", `["[]"]`, raw, `["x"]`, nil, nil))
