...
import (
	...
	"github.com/chendingplano/deepdoc/server/api/ApiUtils"
...
func HandleEmailLogin(w http.ResponseWriter, r *http.Request) {
	...
	stored_password := Databases.GetUserPasswordByEmail(req.Email)
	if stored_password == "" {
...
func HandleEmailVerify(c echo.Context) error {
    ...
    user_info, err := Databases.LookupUserByToken(token)
    if err != nil {
        return c.String(http.StatusBadRequest, "Invalid or expired verification link (MID_EML_119).")
    }

    err1 := Databases.MarkUserVerified(user_info.UserName)
	if err1 == nil {
...
func HandleEmailSignup(c echo.Context) error {
    ...
    // if Databases.UserExists(req.Email) {
	user_status := Databases.GetUserStatus(req.Email)
    ...
    status, err1 := Databases.AddUser(user_info)
	if !status {
		log.Printf("***** Alarm Failed creating user, user_info:%s, err:%w", user_info, err1)
...
func HandleForgotPassword(c echo.Context) error {
    ...
    user, err := Databases.GetUserByEmail(req.Email)
    if err != nil {
        return c.JSON(http.StatusNotFound, map[string]string{"error": "user not found (MID_EML_179)"})
    }

    token := uuid.NewString()
    Databases.UpdateVTokenByEmail(req.Email, token)
...
func HandleResetLink(c echo.Context) error {
    token := c.QueryParam("token")
    _, err := Databases.LookupUserByToken(token)
    if err != nil {
...
func HandleResetPasswordConfirm(c echo.Context) error {
    ...
    user, err := Databases.LookupUserByToken(req.Token)
    if err != nil {
    ...
    err = Databases.UpdatePasswordByUserName(user.UserName, string(hashedPassword))
    if err != nil {
