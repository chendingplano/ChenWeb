...
// modularityReviewer evaluates whether a document's content is **self-contained
// and reusable** — whether each section can stand on its own without forcing the
// reader to hold unrelated context from elsewhere, and whether the same material
// is defined once rather than restated in several places. It flags duplicated
// content, sections that bleed into each other (poor separation of concerns),
// content that cannot be understood without unstated context from a distant
// section, and material that would be better factored into a shared/reusable
// section (e.g. a glossary, a "common definitions" block, an appendix).
// P2, StrategyDocument, one-shot (no tool use). It is the P2 (Structure &
// Organization) reviewer that follows logical_flow, heading_hierarchy,
// navigability, and section_balance.
//
// Like the other P2 document-level reviewers, modularity reasons across the
...
// block boundaries, where a duplicate or a definition may live in a block this
// call cannot see.
type modularityReviewer struct {
...
func (r *modularityReviewer) Name() string             { return "modularity" }
func (r *modularityReviewer) Group() string            { return "P2" }
func (r *modularityReviewer) Strategy() ReviewStrategy { return StrategyDocument }

func (r *modularityReviewer) ReviewDocument(
	ctx context.Context,
	recordID int64,
	cfg ReviewerConfig,
) ([]ReviewFinding, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("(MID_26062451) load record %d: %w", recordID, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("(MID_26062452) resolve line file for record %d: %w", recordID, err)
	}

	body, err := os.ReadFile(lineFilePath)
	if err != nil {
		return nil, fmt.Errorf("(MID_26062453) read line file %s: %w", lineFilePath, err)
	}

	lines, err := ParseInputLinesIncludingTOC(body)
	if err != nil {
		return nil, fmt.Errorf("(MID_26062454) parse line file for record %d: %w", recordID, err)
	}

	if len(lines) == 0 {
		r.logger.Info("modularity review skipped: no lines", "record_id", recordID)
		return nil, nil
	}

	// Build document context for the envelope. The reviewer infers the document
	// type (and thus how self-contained and reusable its sections are expected
	// to be) from this context.
...
func (r *modularityReviewer) processBlock(
	ctx context.Context,
	recordID int64,
	index int,
	total int,
	...
	r.logger.Info("modularity review block start",
		"record_id", recordID,
	...
	payload, err := r.client.ExtractJSON(ctx, newDocReviewLLMJSONInput(ctx, cfg.PromptRef, cfg.PromptText, cfg.ModelName, b.inputJSON, "review_modularity", "MID-CWB-REVIEW-MODULARITY"))
	if err != nil {
		r.logger.Warn("modularity review block failed; skipping",
			"record_id", recordID,
			"block", fmt.Sprintf("%d/%d", index+1, total),
			"error", err,
		)
	...
	for i := range findings {
		...
		if findings[i].FindingType == "" {
			findings[i].FindingType = "modularity"
		}
	...
	r.logger.Info("modularity review block end ",
		"record_id", recordID,
		"block", fmt.Sprintf("%d/%d", index+1, total),
