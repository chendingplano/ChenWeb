...
func RegisterRoutes(e *echo.Echo) error {
	...
	apiGroup.POST("/llm/balances/deposits", llmadminhandler.AddDeposit)
	apiGroup.POST("/llm/balances/total-spending", llmadminhandler.SetTotalSpending)
	apiGroup.GET("/llm/balances/manual-records", llmadminhandler.ListManualRecords)
	apiGroup.GET("/llm/deposit-api-keys", llmadminhandler.ListDepositAPIKeys)
	...
	apiGroup.GET("/kb/semantic-decision-candidates", kbhandler.ListSemanticDecisionCandidates)
	apiGroup.POST("/kb/semantic-decision-candidates", kbhandler.CreateSemanticDecisionCandidate)
	apiGroup.GET("/kb/semantic-decision-candidates/:id", kbhandler.GetSemanticDecisionCandidate)
	apiGroup.POST("/kb/semantic-decision-candidates/:id/transition", kbhandler.TransitionSemanticDecisionCandidate)
	apiGroup.POST("/kb/semantic-decision-candidates/:id/resolution", kbhandler.ResolveSemanticDecisionCandidate)
	...
	apiGroup.POST("/kb/assertion-evidence/:id/delete", kbhandler.DeleteAssertionEvidence)
	apiGroup.POST("/kb/assertion-evidence/:id/restore", kbhandler.RestoreAssertionEvidence)
	...
	apiGroup.POST("/kb/keyword-concepts/:concept_id/unmerge", kbhandler.UnmergeKeywordConcept)
	...
	apiGroup.GET("/kb/keyword-rewrite-rules/admin", kbhandler.AdminListKeywordRewriteRules)
	apiGroup.GET("/kb/keyword-rewrite-rules/admin/scopes", kbhandler.ListKeywordRewriteScopes)
	apiGroup.PUT("/kb/keyword-rewrite-rules/:rule_id", kbhandler.UpdateKeywordRewriteRule)
	...
	apiGroup.GET("/kb/pending-files", kbhandler.ListPendingFiles)
	apiGroup.POST("/kb/pending-files/claim", kbhandler.ClaimPendingFiles)
