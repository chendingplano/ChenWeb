...
ORDER BY b.priority DESC, b.id`)
	mock.ExpectQuery(query).WillReturnRows(sqlmock.NewRows([]string{
		"id", "name", "priority", "predicate", "pipeline_id", "pipeline_name", "active", "create_time", "modify_time",
	}).AddRow(
		int64(2), "pdf-zh", 10, `{"version":1,"expression":{"kind":"all","items":[{"kind":"fact","path":"document.input_doc_type","op":"eq","value":"pdf"},{"kind":"fact","path":"document.source_language","op":"eq","value":"zh"}]}}`, int64(3), "regulated_reference", true,
		time.Date(2026, 7, 31, 10, 0, 0, 0, time.UTC), time.Date(2026, 7, 31, 10, 0, 0, 0, time.UTC),
	).AddRow(
		int64(3), "rich-canonical", 9, `{"version":1,"expression":{"kind":"any","items":[{"kind":"fact","path":"document.input_doc_type","op":"eq","value":"pdf"}]}}`, int64(3), "regulated_reference", true,
		time.Date(2026, 7, 31, 10, 0, 0, 0, time.UTC), time.Date(2026, 7, 31, 10, 0, 0, 0, time.UTC),
	))
...
ORDER BY priority DESC, id`)
	mock.ExpectQuery(gateQuery).WillReturnRows(sqlmock.NewRows([]string{"id", "name", "priority", "predicate", "predicate_checksum", "target_processor", "effect", "required_facets", "active", "create_time", "modify_time"}))
...
WHERE b.binding_kind = 'conditional'
ORDER BY b.priority DESC, b.id`)
	mock.ExpectQuery(legacyQuery).WillReturnRows(sqlmock.NewRows([]string{"id", "name", "priority", "predicate", "pipeline_id", "pipeline_name", "active", "create_time", "modify_time"}))
	gateQuery := regexp.QuoteMeta(`
SELECT id, name, priority, COALESCE(predicate, '{}'::jsonb)::text,
       COALESCE(predicate_checksum, ''), COALESCE(target_processor, ''),
       COALESCE(effect, ''), COALESCE(required_facets, '[]'::jsonb)::text,
...
WHERE target_processor IS NOT NULL
ORDER BY priority DESC, id`)
	mock.ExpectQuery(gateQuery).WillReturnRows(sqlmock.NewRows([]string{"id", "name", "priority", "predicate", "predicate_checksum", "target_processor", "effect", "required_facets", "active", "create_time", "modify_time"}).AddRow(
		int64(12), "skip metrics", 15, `{"version":1,"expression":{"kind":"fact","path":"document.doc_kind","op":"eq","value":"narrative"}}`, "sha256:test", "extract_metrics", "skip", `["document.doc_kind"]`, true, time.Now(), time.Now(),
	))
	c, rec := newPipelineRuleContext(t, http.MethodGet, "/api/v1/kb/pipeline-rules", "")
	if err := ListPipelineRules(c); err != nil {
		t.Fatal(err)
...
`)
	mock.ExpectQuery(insertQuery).
		WithArgs("pdf-zh", 10, sqlmock.AnyArg(), sqlmock.AnyArg(), int64(3), true).
...
RETURNING id`)
	mock.ExpectQuery(insertQuery).
		WithArgs("skip metrics for narrative", 15, sqlmock.AnyArg(), sqlmock.AnyArg(), "extract_metrics", "skip", sqlmock.AnyArg(), true).
...
// TestCreatePipelineRuleRejectsDeferEffect proves ADR 2026081001 DR9: defer
// is no longer a storable gate effect, rejected at creation time before any
// DB write.
func TestCreatePipelineRuleRejectsDeferEffect(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("sqlmock.New failed: %v", err)
	}
...
WHERE b.id = $1 OR b.legacy_rule_id = $1`)
	...
	mock.ExpectQuery(selectCompatQuery).WithArgs(int64(2)).WillReturnRows(sqlmock.NewRows([]string{
		...
		int64(2), "pdf-zh", 20, `{"version":1,"expression":{"kind":"all","items":[{"kind":"fact","path":"document.input_doc_type","op":"eq","value":"pdf"},{"kind":"fact","path":"document.source_language","op":"eq","value":"zh"}]}}`, int64(3), "regulated_reference", false,
...
func TestUpdatePipelineRuleCanonicalGate(t *testing.T) {
	...
	existsQuery := regexp.QuoteMeta(`
SELECT 1 FROM kb.pipeline_rules WHERE id = $1 AND target_processor IS NOT NULL`)
	mock.ExpectQuery(existsQuery).WithArgs(int64(12)).WillReturnRows(sqlmock.NewRows([]string{"exists"}).AddRow(1))
	...
	mock.ExpectExec(updateQuery).WithArgs("skip", 20, int64(12)).WillReturnResult(sqlmock.NewResult(0, 1))
	c, rec := newPipelineRuleIDContext(t, http.MethodPut, "12", `{"effect":"skip","priority":20}`)
	if err := UpdatePipelineRule(c); err != nil {
		t.Fatal(err)
	}
	if rec.Code != http.StatusOK {
	...
	for _, want := range []string{`"id":12`, `"effect":"skip"`} {
		if !strings.Contains(rec.Body.String(), want) {
			t.Fatalf("response %s missing %s", rec.Body.String(), want)
		}
	}
...
// TestUpdatePipelineRuleCanonicalGateRejectsDeferEffect proves ADR
// 2026081001 DR9 applies to updates too, not just creation.
func TestUpdatePipelineRuleCanonicalGateRejectsDeferEffect(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatal(err)
	}
...
SELECT 1 FROM kb.pipeline_rules WHERE id = $1 AND target_processor IS NOT NULL`)
	mock.ExpectQuery(existsQuery).WithArgs(int64(12)).WillReturnRows(sqlmock.NewRows([]string{"exists"}).AddRow(1))
	c, rec := newPipelineRuleIDContext(t, http.MethodPut, "12", `{"effect":"defer"}`)
	if err := UpdatePipelineRule(c); err != nil {
		t.Fatal(err)
	}
	if rec.Code != http.StatusBadRequest {
...
func TestDeletePipelineRuleNotFound(t *testing.T) {
	...
	gateExistsQuery := regexp.QuoteMeta(`
SELECT 1 FROM kb.pipeline_rules WHERE id = $1 AND target_processor IS NOT NULL`)
	mock.ExpectQuery(gateExistsQuery).WithArgs(int64(999)).WillReturnError(sql.ErrNoRows)
