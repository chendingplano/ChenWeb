// Command provision-writer-readiness is the ADR 2026081801 task 7.6/7.7
// precondition check for enabling LOSSLESS_SEMANTIC_WRITES_PROVISION,
// mirroring cmd/metric-writer-readiness's role for the metric gate. It:
//
//  1. runs the shared adapter conformance suite for the provision adapter
//     and records the result in kb.semantic_adapter_compliance (DR13) --
//     this is a real, safe write: recording a compliance result never
//     activates a writer by itself;
...
//
// It never sets LOSSLESS_SEMANTIC_WRITES_PROVISION itself. Flipping that
// gate is a separate, deliberate operational decision this command only
// informs.
//
// Usage:
//
//	PG_DB_NAME=miner provision-writer-readiness
package main
...
func main() {
	...
	adapter, ok := semantic.LookupAdapter(semantic.ProvisionArtifactType)
	if !ok {
		log.Fatal("provision adapter is not registered")
	}
	...
	completeness, err := semantic.CompletenessChecker{DB: db, ArtifactSourceSQL: semantic.ProvisionArtifactSourceSQL}.Run(ctx, adapter)
	if err != nil {
	...
	authErr := semantic.AuthorizeWriterActivation(ctx, db, semantic.ProvisionArtifactType, true)
	fmt.Println()
	if authErr == nil {
	...
	fmt.Println("\nLOSSLESS_SEMANTIC_WRITES_PROVISION was not read or modified by this command.")
